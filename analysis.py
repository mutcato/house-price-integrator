...
df['net_sqm_group'] = pd.cut(df['net_sqm'], bins=[0, 50, 100, 150, 200, float('inf')], labels=['0-50', '51-100', '101-150', '151-200', '200-above'])
...
for i, net_sqm_group in enumerate(average_prices.columns):
    breakpoint()
    plt.plot(average_prices.index, average_prices[net_sqm_group], color=colors[i], label=net_sqm_group)
